 ...
 * 
 */
void WorldTransformation::transform(std::vector<float> &joints_) noexcept(false)
{
    //
    ...
    torch::Tensor cpu_globalTransformations = globalTransformations.to(torch::kCPU);
    for (int i = 0; i < JOINT_NUM ; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            joints_.push_back(cpu_globalTransformations[0][i][j][3].item().toFloat());
        }
    }
